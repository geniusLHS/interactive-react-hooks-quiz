...
이제 위의 규칙이 무엇을 의미하는지 알 수 있다. `React`는 그저 훅이 등장하는 순서대로 리스트에서 값을 찾아 전달해주는 역할을 할 뿐이다. 변수명이 `age`인지 `name`인지는 전혀 중요하지 않다. 훅이 실행되는 순서가 항상 일정하게 유지되는 것이 중요하고, 위의 규칙은 순서가 바뀌는 경우의 예시를 제시한 것 뿐이다.
