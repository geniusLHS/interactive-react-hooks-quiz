...
위치가 동일하고 컴포넌트가 바뀌면 어떻게 될까? 이 경우 리액트는 해당 컴포넌트와 그 하위 트리의 모든 상태를 초기화한다. 이를테면 아래의 컴포넌트는 매 클릭마다 `<input/>`에 입력한 값이 초기화된다. 왜냐하면 매 렌더링마다 다른 `<MyTextField/>` 컴포넌트가 생성되기 때문이다.
