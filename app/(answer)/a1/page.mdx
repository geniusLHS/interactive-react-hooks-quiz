...
그렇지 않다. `count`변수는 그저 **숫자**이다. `Counter` 컴포넌트가 처음으로 렌더링될 때 `count` 변수에는 `useState` 함수에서 가져온 값 `0`이 담긴다. 즉, 처음으로 렌더링된 `Counter` 컴포넌트는 정확히 아래와 같다.
...
`count`변수는 각 렌더링 안에서 변하지 않는 상수이다.

이제 원래의 문제를 다시 보자. `count`의 값이 예를 들어 20이라고 가정하면 해당 렌더링에서의 컴포넌트는 아래와 같다.
