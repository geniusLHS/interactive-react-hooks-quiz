...
그런 생각과 달리, `count`변수는 그저 **숫자**이다. `Counter` 컴포넌트가 처음으로 렌더링될 때 `count` 변수에는 `useState` 함수에서 가져온 값 `0`이 담긴다. 즉, 처음으로 렌더링된 `Counter` 컴포넌트는 정확히 아래와 같다.
