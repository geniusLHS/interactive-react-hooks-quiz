...
`count`의 경우 이전 상태 `0`과 새로운 상태 `0`이 동일하므로 (즉, `Object.is(0,0)`가 `true`) 리렌더링을 건너뛴다. 그러나 `countObj`의 경우 리렌더링을 건너뛸 수 없는데, 이전 상태 `{counter: 0}`과 새로운 상태 `{counter: 0}`가 메모리에서 다른 객체를 참조하기 때문이다.
